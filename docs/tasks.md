...
## Phase 11 — Chunk sanitization and key scoping (`phase-11-chunk-sanitization-and-key-scoping`, base: `main`)

Phases 8–10 (filter-error-mapping, external-call timeouts, `chunk_text` in `/query` response sources — PRs #9, #8, #11) predate this section and aren't being backfilled here; out of scope for this pass, not an oversight.

Closes genai-block7-security's docs/spec.md LLM01 (indirect injection, Path B) and LLM08 (Pinecone key scoping).

- [x] **Sanitized `chunk_text` at ingestion, capped query field lengths, split the Pinecone key by access scope.** New `scripts/sanitize.py`'s `sanitize_chunk_text()` strips role-prefix markers (`System:`/`Human:`/`Assistant:`/`User:`) and chat-template delimiter tokens (`[INST]`, `<|...|>`, `### Instruction`) — not a phrasing blocklist, which is trivially bypassed by rewording. Wired into `ingest.py`'s `main()` before chunks are upserted to Pinecone, chosen over sanitizing inside `generate.py`'s prompt-building step because `chunk_text` also flows back out through `/query`'s `sources[].chunk_text` (added for Block 6 citation support) regardless of whether Claude is ever called — ingestion is the one point that covers every reader. `QueryRequest.question` (the field that actually reaches `generate_answer`'s prompt) got a `max_length=500` cap; `condition`/`drug`/`lab` (Pinecone metadata filter values only, validated against fixed whitelists, never reaching a prompt) got narrower caps as input hygiene, not injection defense. Split `PINECONE_API_KEY` (full access, kept for `create_index.py`/`ingest.py`/`verify.py`) from a new `PINECONE_QUERY_API_KEY` (scoped to DataPlaneViewer, query-only, used by `retrieve.py`/the live API) — so a compromise of the query path, the one this API exposes externally, can't write or delete index data. `check_connection.py`'s smoke test extended with `check_pinecone_query_key()` (a data-plane `describe_index_stats()` call, since the scoped key can't call the control-plane `list_indexes()` the existing check uses) and wired into both `check_connection.py.main()` and `verify.py`'s `check_credentials()`, so the split key doesn't silently stop being covered by either. Tests added: 4 over-length-field cases in `test_api.py` (422 for each of `question`/`condition`/`drug`/`lab` individually, with the other fields valid so only the oversized one triggers the rejection); an end-to-end prompt-cleanliness test in new `tests/test_generate.py`, planting an injection payload in a synthetic note, sanitizing it the way `ingest.py` does, and asserting none of the structural markers survive into the prompt Claude receives; and a live-credential integration test in new `tests/test_pinecone_key_scope.py` proving the query-scoped key gets a real 403 from Pinecone itself on an upsert and a delete call — skipped by default (no CI secret should ever hold a key whose entire point is "can't do something destructive"), meant to run manually after creating the scoped key in the Pinecone console.
- [x] **Widened `condition`/`lab`'s `max_length` to match Block 8's external `QueryRequest` bounds.** Initial caps (`condition=100`, `lab=20`) were sized only against this repo's own real data (longest observed condition/drug name 25/19 chars; `lab`'s whitelist entries top out at 7). But Block 8's `app/api.py` is this API's real external entry point, with its own `QueryRequest` validation running first (`condition=200`, `lab=100`, matching `drug_a`/`drug_b=100`) — a request Block 8 already accepted could otherwise still hit a confusing 422 several layers downstream here. Widened `condition` to 200 and `lab` to 100 to match Block 8 exactly; costs nothing in practice since real values are still comfortably covered.
- [x] **Closed three real gaps in the role-marker regex, mirroring Block 6's identical fixes to the same underlying pattern (found first on that repo's `citation_sanitization.py`, ported back here).** (1) Anchored to start-of-line only, so a marker spliced into one continuous sentence (no newline — the realistic shape for this corpus) went undetected; matched after sentence-ending punctuation too. (2) Substitution removed the whitespace before the marker along with it, fusing the following sentence onto the preceding one and erasing the sentence boundary downstream logic depends on; now substitutes with a single space instead of empty string. (3) Still missed a marker after a comma or plain mid-sentence space; unanchored to `\b(...)\s*:\s*`, matching Block 6's fix exactly. Tests added for each closed gap in `tests/test_sanitize.py` and `tests/test_generate.py`.
- [x] 46 passed, 2 skipped (the two live-credential Pinecone key-scope tests, no live key configured in this run) — full suite
- [x] Push `phase-11-chunk-sanitization-and-key-scoping`, open PR against `main` (PR #13, open)
